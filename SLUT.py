    ...
    def reconstruct_luts(self):
        ...
        if self.mode == "--":
            ...
            if self.mode == "+-":
                ...
                CUBEs = self.s_Layers.mm(self.LUTs).reshape(dim, num * 3, dim * dim).permute(1, 0, 2).reshape(num, 3,
                                                                                                              self.dim,
                                                                                                              self.dim,
                                                                                                              self.dim)
            if self.mode == "-+":
            ...
            if self.mode == "++":
                ...
                CUBEs = self.s_Layers.mm(self.LUTs.mm(self.w_Layers).reshape(-1, num * 3 * dim * dim)).reshape(dim,
                                                                                                               num * 3,
                                                                                                               dim ** 2).permute(
                    1, 0, 2).reshape(num, 3, self.dim, self.dim, self.dim)
...
class HashLUT(nn.Module):
    def __init__(self, nt="7+13", backbone='Backbone', compensate=True, min_max='9+64', *args, **kwargs):
        ...
        if compensate:
            self.hashluts = tcnn.NetworkWithInputEncoding(n_input_dims=3, n_output_dims=self.N * 3,
    ...
    def forward(self, img, img_org, *args, **kwargs):
        ...
        mid_results = self.hashluts(img_org.reshape(-1, 3)).reshape(B, H * W, self.N, -1)[...,
                      :3]  # B, HW, N, 3
